 ...
 * 品牌商品列表
 */
  getProductsByBrandId: function (obj, brand_id) {
    var that = this;
    that.loading();
      ...
      if (!that.errorRes(res, obj)) {
        ...
        zcjy.request(that, '/api/products_of_cat_with_children/' + cat_id, function (res2) {
          if (!that.errorRes(res2, obj)) {
            ...
            that.startManyImagesLoad(obj.data.currentProduct, obj);
            that.hideLoading();
          }
  ...
  getProductsByCatId: function (obj, cat_id, skip=0, take = this.conf.pageTake){
    ...
    zcjy.request(that, '/api/products_of_cat_with_children/' + cat_id, function (res2) {
      if (!that.errorRes(res2, obj)) {
        //that.conf.category.currentProduct = res2.data.data;
        obj.setData({
          currentProduct: that.autoPushData(that.conf.category.currentProduct,res2.data.data),
  ...
  getCountriesProducts: function (obj,country_id,skip = 0, take = this.conf.pageTake){
    ...
    zcjy.request(that, '/api/country_products/' + country_id, function (res) {
      if (!that.errorRes(res)) {
        obj.setData({
          country_products: res.data.data,
          'themeData.country_products': res.data.data
        });
        that.startManyImagesLoad(obj.data.country_products, obj);
      }
    });
  },
  /**
 ...
 */
  ...
  //活动详情
  productPrompDetail: function (obj,id) {
    zcjy.request(that, '/api/product_proms/'+id, function (res) {
      if (!that.errorRes(res)) {
          obj.setData({
              product_proms_list:res.data.data
          });
      }
    });
  },
  ...
  conf: {
    ...
    server: 'https://shop-model.yunlike.cn',
    //小程序appid  'wx29099c7dc078af23'
    appid: 'wx29099c7dc078af23',
    //小程序secret 'cbea2a5a385b72c0cfaf4708e17c4b7f',
    // secret:'a2061cd4ddd846c22161939f08e6b25b',
    //首页
    index: {
      //首页横幅
      indexBanner: null,
